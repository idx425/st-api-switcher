...
- 📁 **分组收纳**：点选芯片给站点归组（或一键新建分组），同组收进可折叠的分组框，折叠状态记忆，站点再多也不乱
- ⚡ **一键切换**：点「使用」自动写入 Key → 切换到自定义(OpenAI 兼容)接口 → 填入 URL/模型 → 自动连接
- 🖥️ **获取模型列表**：从站点接口拉取全部模型，搜索选择，不用手打（并自动保护当前连接的 Key 不被污染）
- 🪄 **魔棒快捷面板**：聊天输入框旁的魔棒菜单 → API 快切，按分组展示，聊天中随手切换
...
3. 切换：点站点卡片上的 **使用**，或魔棒菜单 → API 快切，或 `/apiswitch 站点名`
